...
read_aggregated_piwick_file <- function(aggregated_piwick_file) {
  ...
  urls <- urls %>%
    ...
    group_by(date, url) %>%
    summarise(nb_visits = sum(nb_visits), nb_uniq_visitors = sum(nb_uniq_visitors), entry_nb_visits = sum(entry_nb_visits), entry_nb_uniq_visitors = sum(entry_nb_uniq_visitors), entry_bounce_count = sum(entry_bounce_count), exit_nb_visits = sum(exit_nb_visits), exit_nb_uniq_visitors = sum(exit_nb_uniq_visitors), avg_time_on_page = mean(avg_time_on_page)) %>% #We merge any duplicate and sum the relevant values
    ungroup()
...
complete_time <- function(aggregated_frequent) {
  ...
  aggregated_frequent <- aggregated_frequent %>% group_by(url) %>% complete(date = time_sequence, fill = list(nb_visits = 0)) %>% ungroup()
  return(aggregated_frequent)
}

#Function to set up a logs_dataset object containing:
#*The original data
...
get_aggregated_piwick <- function(aggregated_piwick_file, absolute_days=10, relative_days=0.9) {
  ...
  aggregated_piwick_visits_frequent <- aggregated_piwick_visits %>% filter(selected == "yes") %>% select(-selected)
  
  aggregated_piwick_visits_seldom <- aggregated_piwick_visits %>% filter(selected == "no") %>% select(-selected)
  
  aggregated_piwick_visits_frequent <- complete_time(aggregated_piwick_visits_frequent)
  
  aggregated_piwick_visits_frequent <- aggregated_piwick_selected %>% left_join(aggregated_piwick_visits_frequent)
