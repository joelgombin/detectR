...
get_calendar_time_series <- function(logs_board){
  ...
  complete_time <- time_serie_board(all_per_day=all_per_day, weekday_values = wday_values, models_time = models_time, outliers_time=outliers_time)
...
get_trend_model <- function(time_decomposition, all_per_day){
  ...
  trend <- trend %>% filter(!is.na(trend))
...
get_outliers <- function(all_per_day) {
  ...
  cooksd <- cooksd %>% mutate(day_publication = as.numeric(day_publication)) %>% mutate(replacement_day=ifelse(day_publication==nrow(cooksd), day_publication-1, day_publication+1))
  ...
  cooksd <- cooksd %>% mutate(corrective_value = 1+(1-original_value/replacement_value))
  return(cooksd)
}
