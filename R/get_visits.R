...
get_visits <- function(conn, table = "visites_par_jour", visited_urls = NULL, from = "2017-01-01", to = Sys.Date()) {
  ...
  if (is.null(visited_urls)) {
    request <- glue::glue_sql("SELECT * FROM {`table`} WHERE {`date`} >= {from} AND {`date`} <= {to}", .con = conn)
  } else {
    request <- glue::glue_sql("SELECT * FROM {`table`} WHERE url IN ({visited_urls*}) AND {`date`} >= {from} AND {`date`} <= {to}", .con = conn)
